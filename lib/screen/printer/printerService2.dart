...
class PrinterService2 {
  const PrinterService2();
  ...
  Future<void> print(Printer printer) async {
    ...
    for (var i = 0; i < printer.confirmOrder!.orders!.length; i++) {
      ...
      for (var j = 0; j < printer.confirmOrder!.orders![i].order_lines!.length; j++) {
        int qty = int.parse('${printer.confirmOrder!.orders![i].order_lines![j].qty}');
        double price = double.parse('${printer.confirmOrder!.orders![i].order_lines![j].price_per_unit}');
        double price_per_unit = price * qty;
        await SunmiPrinter.printRow(cols: [
          ColumnMaker(
            text: '${printer.confirmOrder!.orders![i].order_lines![j].product!.code}',
            width: 27,
            align: SunmiPrintAlign.LEFT,
          ),
          ColumnMaker(
            text: '${printer.confirmOrder!.orders![i].order_lines![j].qty}',
            width: 6,
            align: SunmiPrintAlign.RIGHT,
          ),
          ColumnMaker(
